...
export const OfferCineBlockV2: FC<Props> = ({ title, onSeeVenuePress, offer }) => {
  const theme = useTheme()
  const { data: movieCalendar, isLoading } = useOfferMovieCalendarQuery({ offer })
  const { calendar } = movieCalendar ?? { calendar: [] }
  const calendarDates = calendar.map((dayVenuScreenings) => new Date(dayVenuScreenings.date))
